    ...
    <AuroraBackground>
      ...
      >
        <h1 className="text-3xl md:text-7xl font-bold text-center font-serif text-burgundy-900">
          {dict.hero.title} <br /> <span className="text-burgundy-700">{dict.hero.titleHighlight}</span>
        </h1>
        <p className="font-light text-base md:text-2xl text-neutral-600 py-4 text-center max-w-2xl">
          {dict.hero.subtitle}
        </p>
        <div className="flex gap-4">
          <Link href={`/${lang}/contact`} className="bg-burgundy-700 rounded-full w-fit text-white px-6 py-3 font-medium hover:bg-burgundy-800 transition-colors shadow-lg hover:shadow-xl hover:scale-105 transform duration-200">
            {dict.hero.cta}
          </Link>
